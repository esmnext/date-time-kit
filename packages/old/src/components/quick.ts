...
export function create(data: kitContent) {
    ...
    ele.querySelector('.dt-quick-item-active')?.classList.remove(
        'dt-quick-item-active'
    );
    const limitKey = getLimitKey(data);
    if (limitKey)
        ele.querySelector(
            '.dt-quick-item[data-limit="' + limitKey + '"]'
        )?.classList.add('dt-quick-item-active');
    // add event
    ele.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('dt-quick-item')) {
            target.parentElement
                ?.querySelector('.dt-quick-item-active')
                ?.classList.remove('dt-quick-item-active');
        ...
        if (
            target.matches(
                '.dt-time-zone, .dt-time-zone-text, .dt-time-zone-icon'
            )
        ) {
            if (ele.classList.contains('dt-quick-zone-disabled')) return;
            ele.querySelector('.dt-time-zone')?.classList.add(
                'dt-time-zone-select-show'
            );
            return;
        }

        if (target.classList.contains('dt-time-zone-mask')) {
            ele.querySelector('.dt-time-zone')?.classList.remove(
                'dt-time-zone-select-show'
            );
            return;
        }
        ...
        if (zoneItem.classList.contains('dt-time-zone-item')) {
            ele.querySelector('.dt-time-zone-select-active')?.classList.remove(
...
function setDate(limit: kitDataLimit, data: kitContent) {
    ...
    if (
        endData.getFullYear() === startData.getFullYear() &&
        endData.getMonth() === startData.getMonth()
    ) {
...
function render(data: kitContent) {
    ...
    for (const key in QUICK_MAP) {
        ...
        if (
            data.maxLength &&
            (!limitData || limitData.length >= data.maxLength)
        )
            continue;
...
function renderTimeZoneList(data: kitContent) {
    ...
    const allZones = [
        -12, -11, -10, -9.5, -9, -8, -7, -6, -5, -4, -3, -3.5, -2, -1, 0, 1, 2,
        3, 3.5, 4, 4.5, 5, 5.5, 5.75, 6, 6.5, 7, 8, 8.75, 9, 9.5, 10, 10.5, 11,
        12, 12.45, 13, 14
    ];
...
export function getQuickMap(): Record<
    kitDataLimit,
    kitDataLimitContent | null
> {
    return {
        all: null,
        today: getTodayLimit(),
...
function limitFactory(startTime = new Date(), endTime = startTime) {
    ...
    result.length =
        new Date(
            utils.kitDateAndTime2timeStr(result.endDate, result.endTime)
        ).getTime() -
        new Date(
            utils.kitDateAndTime2timeStr(result.startDate, result.startTime)
        ).getTime();
    return result;
}
...
function getWeekLimit() {
    ...
    const endTime = new Date(
        current.getFullYear(),
        current.getMonth(),
        current.getDate() - current.getDay() + 7
    );
...
function getLastWeekLimit() {
    ...
    const startTime = new Date(
        current.getFullYear(),
        current.getMonth(),
        current.getDate() - current.getDay() - 6
    );
    const endTime = new Date(
        current.getFullYear(),
        current.getMonth(),
...
export function getLimitKeyByTimestamp(
    ...
    const startTimeDate = new Date(
        utils.getTimeStringByTimeZone(startTimestamp, currentTimeZone, timeZone)
    );
    const endTimeDate = new Date(
        utils.getTimeStringByTimeZone(endTimestamp, currentTimeZone, timeZone)
    );
...
export function getTimestampByLimitKey(
    ...
    result.startTime = utils.kitDateAndTime2timeStr(
        startDateTime.date,
        startDateTime.time
    );
    result.endTime = utils.kitDateAndTime2timeStr(
        endDateTime.date,
        endDateTime.time
    );
