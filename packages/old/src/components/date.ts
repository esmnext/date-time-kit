...
import './date.scss';
import i18n, { type kitI18nCfg } from '../i18n';
...
export function create(data: kitContent, status: status = 'start') {
    ...
    ele.addEventListener('click', (e) => {
        ...
        if (
            !element ||
            element.matches('.dt-data-item-disabled, .dt-data-item-dis-sel')
        )
            return;
    ...
    ele.addEventListener('mousemove', (e) => {
        ...
        const element = e.target.closest('.dt-data-item');
        if (!element || element.matches('.dt-data-item-disabled')) return;
...
function render(data: kitContent, status: status) {
    ...
    return `${renderWeekTitle(
        i18n[data.lang].date
    )}<div class="dt-date-content" data-date="${utils.getDateTimeStr(
        renderDate?.getFullYear(),
        renderDate?.getMonth() + 1
    )}">${dataHTML}</div>`;
}
...
function renderDayByMonth(data: kitContent, date: Date) {
    ...
    const currentString = utils.getDateTimeStr(
        current.getFullYear(),
        current.getMonth() + 1,
        current.getDate()
    );
...
></div>`;
    ...
    const minDateObj = new Date(
        utils.kitDateAndTime2timeStr(data.minDate, timeTem)
    );
    const maxDateObj = new Date(
        utils.kitDateAndTime2timeStr(data.maxDate, timeTem)
    );
    ...
    for (let i = 1; i <= days; i++) {
        ...
        if (data.period && data.startDate.year && data.endDate.year) {
            if (
                new Date(startDate) < renderDateObj &&
                new Date(endDate) > renderDateObj
            ) {
        ...
        if (
            minDateObj.getTime() > renderDateObj.getTime() ||
        ...
        if (data.period && data.startDate.year && data.moveDate.year) {
            ...
            const maxLength =
                data.maxLength < dayLength ? dayLength : data.maxLength;
            // subtract 1 day because the selected day is also counted
            const minLength =
                data.minLength < dayLength ? dayLength : data.minLength;
            // minLength -= dayLength;
            ...
            if (
                data.maxLength &&
                startTimeStamp.getTime() + maxLength < renderDateObj.getTime()
            ) {
                classList.push('dt-data-item-disabled');
            }
            if (
                data.maxLength &&
                endTimeStamp.getTime() - maxLength > renderDateObj.getTime()
            ) {
                classList.push('dt-data-item-disabled');
            }
            if (
                data.minLength &&
                startTimeStamp.getTime() + minLength >
                    renderDateObj.getTime() &&
