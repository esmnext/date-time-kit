...
>(delay = 0) {
    return (target: Fn, _ctx: ClassMethodDecoratorContext<This, Fn>) =>
        debounce(target, delay) as Fn;
}
 ...
 */
export const initKitDate = (date?: Date): kitDate =>
    !date
        ? {
              year: 0,
              month: 0,
 ...
 */
export const initKitTime = (date?: Date): kitTime =>
    !date
        ? {
              hour: 0,
              minute: 0,
 ...
 */
export const getDateTimeStr = <T extends number | undefined = undefined>(
    year: number,
    month: number,
    date?: T
) =>
    `${year.toString().padStart(4, '0')}-${month.toString().padStart(2, '0')}${
        date === void 0 ? '' : `-${date.toString().padStart(2, '0')}`
...
export const getTimeStringInSeconds = (
    hour: number,
    minute: number,
    second: number
) =>
...
export const kitTime2timeStr = <T extends true | undefined = undefined>(
    time: kitTime,
    withoutMs?: T
) =>
    (withoutMs
...
export const kitDateAndTime2timeStr = (
    date: kitDate,
    time: kitTime
): timeString =>
...
export function dataFactory(opts: kitOption): kitContent {
    ...
    maxTime =
        minTime ||
        getTimeStringByTimestamp(now - 1000 * 60 * 60 * 24 * 365 * 5);
    minTime =
        temTime ||
        getTimeStringByTimestamp(now + 1000 * 60 * 60 * 24 * 365 * 20);

    if (opts.timeZone !== void 0) {
        const currentTimeZone = getCurrentTimeZone();
        maxTime =
            maxTime ||
            getTimeStringByTimeZone(maxTime, currentTimeZone, opts.timeZone);
        minTime =
            minTime ||
            getTimeStringByTimeZone(minTime, currentTimeZone, opts.timeZone);
        startTime =
            startTime &&
            getTimeStringByTimeZone(startTime, currentTimeZone, opts.timeZone);
        endTime =
            endTime &&
            getTimeStringByTimeZone(endTime, currentTimeZone, opts.timeZone);
    }
    ...
    if (
        ...
        startDateShow.getMonth() === endDate.getMonth()
    ) {
    ...
    // init time zone
    const timeZone =
        opts.timeZone !== void 0 ? opts.timeZone : getCurrentTimeZone();
...
export function getTimeStringByTimeZone(
    ...
    const result = getDateByTimeZone(
        new Date(datetime),
        targetTimeZone,
        currentTimeZone
    );
