...
export const create = ({ root }: kitOption, data: kitContent) =>
    new Promise<kitResultPeriod | kitResultSingle>((resolve, reject) => {
        ...
        const dataProxy = new Proxy(data, {
            set(target, _key, value) {
                ...
                if (typeof target[key] === 'object') {
                    ...
                    const isSame = Object.keys(dateOrTime).every(
                        (k) =>
                            dateOrTime[k as keyof typeof dateOrTime] ===
                            value[k]
                    );
        ...
        function done() {
            ...
            const startTime = utils.kitDateAndTime2timeStr(
                startDate.date,
                startDate.time
            );

            if (data.period) {
                const endTime = utils.kitDateAndTime2timeStr(
                    endDate.date,
                    endDate.time
                );
                resolve({
